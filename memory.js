  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
          ...
          fs.readFile("/proc/meminfo", function (error, stdout) {
            if (!error) {
              ...
              let available = parseInt(
                util.getValue(lines, "memavailable"),
                10
              );
              result.available = available
                ? available * 1024
                : result.free + result.buffcache;
              result.active = result.total - result.available;

              result.swaptotal = parseInt(
                util.getValue(lines, "swaptotal"),
                10
              );
              ...
              result.writeback = parseInt(
                util.getValue(lines, "writeback"),
                10
              );
            ...
            function (error, stdout) {
              if (!error) {
                ...
                const pagesize = parseInt(
                  util.getValue(lines, "vm.stats.vm.v_page_size"),
                  10
                );
                const inactive =
                  parseInt(
                    util.getValue(lines, "vm.stats.vm.v_inactive_count"),
                    10
                  ) * pagesize;
                const cache =
                  parseInt(
                    util.getValue(lines, "vm.stats.vm.v_cache_count"),
                    10
                  ) * pagesize;
                ...
                if (isNaN(result.total)) {
                  result.total = parseInt(
                    util.getValue(lines, "hw.physmem"),
                    10
                  );
                }
                result.free =
                  parseInt(
                    util.getValue(lines, "vm.stats.vm.v_free_count"),
                    10
                  ) * pagesize;
      ...
      if (_darwin) {
        let pageSize = 4096;
        try {
          let sysPpageSize = util.toInt(
            execSync("sysctl -n vm.pagesize").toString()
          );
        ...
        try {
          exec(
            'vm_stat 2>/dev/null | grep "Pages active"',
            function (error, stdout) {
              if (!error) {
                let lines = stdout.toString().split("\n");
                    ...
                    if (lines.length > 0) {
                      ...
                      lineArray.forEach((line) => {
                        if (line.toLowerCase().indexOf("total") !== -1) {
                          result.swaptotal =
                            parseFloat(line.split("=")[1].trim()) * 1024 * 1024;
                        }
...
function memLayout(callback) {
  ...
  function getManufacturerLinux(manId) {
    ...
    if (
      manIdSearch.length === 4 &&
      {}.hasOwnProperty.call(LINUX_RAM_manufacturers, manIdSearch)
    ) {
    ...
    process.nextTick(() => {
            ...
            if (!error) {
              ...
              devices.forEach(function (device) {
                ...
                if (parseInt(util.getValue(lines, "Size"), 10) > 0) {
                  const totalWidth = util.toInt(
                    util.getValue(lines, "Total Width")
                  );
                  const dataWidth = util.toInt(
                    util.getValue(lines, "Data Width")
                  );
                  result.push({
                    size,
                    bank,
                    ...
                    ecc:
                      dataWidth && totalWidth ? totalWidth > dataWidth : false,
                    clockSpeed: util.getValue(lines, "Configured Clock Speed:")
                      ? parseInt(
                          util.getValue(lines, "Configured Clock Speed:"),
                          10
                        )
                    ...
                    manufacturer: getManufacturerLinux(
                      util.getValue(lines, "Manufacturer:")
                    ),
                    partNum: util.getValue(lines, "Part Number:"),
                    serialNum: util.getValue(lines, "Serial Number:"),
                    voltageConfigured:
                      parseFloat(util.getValue(lines, "Configured Voltage:")) ||
                      null,
                    voltageMin:
                      parseFloat(util.getValue(lines, "Minimum Voltage:")) ||
                      null,
                    voltageMax:
                      parseFloat(util.getValue(lines, "Maximum Voltage:")) ||
                      null,
                  });
                } else {
                  result.push({
            ...
            if (!result.length) {
              ...
              try {
                ...
                if (
                  ...
                  result[0].type =
                    version && version[2] && version[2] === "3"
                      ? "LPDDR4"
                      : result[0].type;
                  result[0].ecc = false;
                  result[0].clockSpeed =
                    (version && version[2] && clockSpeed[version[2]]) || 400;
                  result[0].clockSpeed =
                    version && version[4] && version[4] === "d"
                      ? 500
                      : result[0].clockSpeed;
                  ...
                  let freq =
                    parseInt(
                      util.getValue(lines, "sdram_freq", "=", true),
                      10
                    ) || 0;
                  if (freq) {
                  ...
                  lines = stdout.toString().split("\n");
                  let voltage =
                    parseFloat(util.getValue(lines, "volt", "=", true)) || 0;
                  if (voltage) {
      ...
      if (_darwin) {
        exec("system_profiler SPMemoryDataType", function (error, stdout) {
          if (!error) {
            ...
            const eccStatus = util
              .getValue(allLines, "ecc", ":", true)
              .toLowerCase();
            ...
            devices.forEach(function (device) {
              let lines = device.split("\n");
              const bank =
                (hasBank ? "BANK " : "DIMM") + lines[0].trim().split("/")[0];
      ...
      if (_windows) {
        ...
        try {
          util
            .powerShell(
              "Get-CimInstance Win32_PhysicalMemory | select DataWidth,TotalWidth,Capacity,BankLabel,MemoryType,SMBIOSMemoryType,ConfiguredClockSpeed,FormFactor,Manufacturer,PartNumber,SerialNumber,ConfiguredVoltage,MinVoltage,MaxVoltage | fl"
            )
            .then((stdout, error) => {
              if (!error) {
                ...
                devices.forEach(function (device) {
                  let lines = device.split("\r\n");
                  const dataWidth = util.toInt(
                    util.getValue(lines, "DataWidth", ":")
                  );
                  const totalWidth = util.toInt(
                    util.getValue(lines, "TotalWidth", ":")
                  );
                  const size =
                    parseInt(util.getValue(lines, "Capacity", ":"), 10) || 0;
                  if (size) {
                    result.push({
                      size,
                      bank: util.getValue(lines, "BankLabel", ":"), // BankLabel
                      type: memoryTypes[
                        parseInt(util.getValue(lines, "MemoryType", ":"), 10) ||
                          parseInt(
                            util.getValue(lines, "SMBIOSMemoryType", ":"),
                            10
                          )
                      ],
                      ecc:
                      ...
                      clockSpeed:
                        parseInt(
                          util.getValue(lines, "ConfiguredClockSpeed", ":"),
                          10
                        ) ||
                        parseInt(util.getValue(lines, "Speed", ":"), 10) ||
                        0,
                      formFactor:
                        FormFactors[
                          parseInt(
                            util.getValue(lines, "FormFactor", ":"),
                            10
                          ) || 0
                        ],
                      ...
                      voltageConfigured:
                        (parseInt(
                          util.getValue(lines, "ConfiguredVoltage", ":"),
                          10
                        ) || 0) / 1000.0,
                      voltageMin:
                        (parseInt(
                          util.getValue(lines, "MinVoltage", ":"),
                          10
                        ) || 0) / 1000.0,
                      voltageMax:
                        (parseInt(
                          util.getValue(lines, "MaxVoltage", ":"),
                          10
                        ) || 0) / 1000.0,
                    });
                  }
