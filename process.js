    ...
    process.nextTick(() => {
      ...
      cpuFlags().then((flags) => {
        ...
        result.virtualization =
          flags.indexOf("vmx") > -1 || flags.indexOf("svm") > -1;
        if (_darwin) {
          exec(
            ...
            function (error, stdout) {
              ...
              let tbFrequency =
                util.getValue(lines, "hw.tbfrequency") / 1000000000.0;
              ...
              result.speedMin = util.getValue(lines, "hw.cpufrequency_min")
                ? util.getValue(lines, "hw.cpufrequency_min") / 1000000000.0
                : result.speed;
              result.speedMax = util.getValue(lines, "hw.cpufrequency_max")
                ? util.getValue(lines, "hw.cpufrequency_max") / 1000000000.0
                : result.speed;
              result.vendor =
                util.getValue(lines, "machdep.cpu.vendor") || "Apple";
              result.family =
        ...
        if (_linux) {
          ...
          exec(
            ...
            function (error, stdout) {
              ...
              const modellineParts = modelline.split("@");
              result.brand = modellineParts[0].trim();
              result.speed = modellineParts[1]
                ? parseFloat(modellineParts[1].trim())
                : 0;
              if (
                result.speed === 0 &&
              ...
              result.speedMin =
                Math.round(
                  parseFloat(
                    util.getValue(lines, "cpu min mhz").replace(/,/g, ".")
                  ) / 10.0
                ) / 100;
              result.speedMax =
                Math.round(
                  parseFloat(
                    util.getValue(lines, "cpu max mhz").replace(/,/g, ".")
                  ) / 10.0
                ) / 100;

              result = cpuBrandManufacturer(result);
              result.vendor = cpuManufacturer(
                util.getValue(lines, "vendor id")
              );
              ...
              const threadsPerCore =
                util.getValue(lines, "thread(s) per core") || "1";
              ...
              const coresPerSocket = parseInt(
                util.getValue(lines, "core(s) per socket"),
                10
              ); // number of cores (e.g. 16 on i12900)
              result.physicalCores = coresPerSocket
                ? coresPerSocket * processorsInt
                : result.cores / threadsPerCoreInt;
              result.performanceCores =
                threadsPerCoreInt > 1
                  ? result.cores - result.physicalCores
                  : result.cores;
          ...
          exec(
            ...
            function (error, stdout) {
              ...
              if (!error) {
                ...
                cache =
                  data.length > 2 ? data[2].split("Cache Information") : [];
              ...
              result.speedMax =
                Math.round(
                  parseFloat(
                    util.getValue(lines, "max speed").replace(/Mhz/g, "")
                  ) / 10.0
                ) / 100;
              ...
              for (let i = 0; i < cache.length; i++) {
                ...
                const sizeParts = util
                  .getValue(lines, "Installed Size")
                  .split(" ");
          ...
          try {
            ...
            Promise.all(workload).then((data) => {
              ...
              if (name.indexOf("@") >= 0) {
                ...
                result.speed = name.split("@")[1]
                  ? parseFloat(name.split("@")[1].trim())
                  : 0;
                _cpu_speed = result.speed;
              } else {
              ...
              const countThreads = util.getValue(
                lines,
  ...
  if (cpus && cpus.length) {
    for (let i in cpus) {
      if ({}.hasOwnProperty.call(cpus, i)) {
        let freq =
          cpus[i].speed > 100 ? (cpus[i].speed + 1) / 1000 : cpus[i].speed / 10;
          ...
          if (parts.length === 2) {
            ...
            for (let i = 0; i < lines.length; i++) {
              ...
              if (line.startsWith("acpi") && lines2[i]) {
                result.socket.push(
                  Math.round(parseInt(lines2[i], 10) / 100) / 10
                );
              }
          ...
          exec(cmd, function (error, stdout) {
            ...
            lines.forEach((line) => {
              ...
              if (
                value &&
                (label === undefined ||
                  (label && label.toLowerCase().startsWith("core")))
              ) {
          ...
          exec(
            ...
            function (error, stdout) {
              if (!error) {
                ...
                let flag_bin =
                  "0".repeat(32 - flag_bin_unpadded.length) + flag_bin_unpadded;
          ...
          exec(
            ...
            function (error, stdout) {
              if (!error) {
                ...
                lines.forEach(function (line) {
                  if (
                    line.split(":")[0].toUpperCase().indexOf("FLAGS") !== -1
                  ) {
        ...
        exec(
          ...
          function (error, stdout) {
            ...
            if (!error) {
              ...
              const lines =
                parts.length > 1
                  ? parts[1].split("\tVersion:")[0].split("\n")
                  : [];
              lines.forEach(function (line) {
                let flag = (
                  line.indexOf("(") ? line.split("(")[0].toLowerCase() : ""
                )
                  .trim()
                  .replace(/\t/g, "");
                if (flag) {
      ...
      if (_darwin) {
        exec("sysctl machdep.cpu.features", function (error, stdout) {
          if (!error) {
            ...
            if (
              lines.length > 0 &&
              lines[0].indexOf("machdep.cpu.features:") !== -1
            ) {
    ...
    process.nextTick(() => {
      ...
      if (now >= 200) {
        ...
        for (let i = 0; i < _corecount; i++) {
          ...
          let tmpTick =
            _cpus && _cpus[i] && _cpus[i].totalTick ? _cpus[i].totalTick : 0;
          let tmpLoad =
            _cpus && _cpus[i] && _cpus[i].totalLoad ? _cpus[i].totalLoad : 0;
          ...
          let tmpSteal =
            _cpus && _cpus[i] && _cpus[i].steal ? _cpus[i].steal : 0;
          let tmpGuest =
            _cpus && _cpus[i] && _cpus[i].guest ? _cpus[i].guest : 0;
          _cpus[i] = cpu;
          _cpus[i].totalTick =
            _cpus[i].user +
            _cpus[i].sys +
          ...
          cores[i].loadSystem =
            (_cpus[i].loadSystem / _cpus[i].currentTick) * 100;
          cores[i].loadNice = (_cpus[i].loadNice / _cpus[i].currentTick) * 100;
          cores[i].loadIdle = (_cpus[i].loadIdle / _cpus[i].currentTick) * 100;
          cores[i].loadIrq = (_cpus[i].loadIrq / _cpus[i].currentTick) * 100;
          cores[i].loadSteal =
            (_cpus[i].loadSteal / _cpus[i].currentTick) * 100;
          cores[i].loadGuest =
            (_cpus[i].loadGuest / _cpus[i].currentTick) * 100;
          ...
          cores[i].model = os.cpus()[i].model;
          cores[i].speed = os.cpus()[i].speed;
        }
        let totalTick =
          totalUser +
          totalSystem +
        ...
        result = {
          ...
          currentLoadUser:
            ((totalUser - _current_cpu.user) / currentTick) * 100,
          currentLoadSystem:
            ((totalSystem - _current_cpu.system) / currentTick) * 100,
          currentLoadNice:
            ((totalNice - _current_cpu.nice) / currentTick) * 100,
          currentLoadIdle:
            ((totalIdle - _current_cpu.idle) / currentTick) * 100,
          currentLoadIrq: ((totalIrq - _current_cpu.irq) / currentTick) * 100,
          currentLoadSteal:
            ((totalSteal - _current_cpu.steal) / currentTick) * 100,
          currentLoadGuest:
            ((totalGuest - _current_cpu.guest) / currentTick) * 100,
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (cpus && cpus.length) {
        ...
        let totalTicks =
          totalIdle + totalIrq + totalNice + totalSystem + totalUser;
