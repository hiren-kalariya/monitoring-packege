...
function parseElapsedTime(etime) {
  ...
  let result =
    ...
    " " +
    res.toISOString().substring(11, 19);
  try {
    ...
    process.nextTick(() => {
      ...
      if (srv) {
        ...
        if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
          if (
            (_linux || _freebsd || _openbsd || _netbsd) &&
            srvString === "*"
          ) {
            try {
              ...
              try {
                srvString = "";
                const tmpsrv = execSync("service --status-all 2> /dev/null")
                  .toString()
                  .split("\n");
                for (const s of tmpsrv) {
                  const parts = s.split("]");
                  if (parts.length === 2) {
                    srvString +=
                      (srvString !== "" ? "|" : "") + parts[1].trim();
                  }
                }
                srvs = srvString.split("|");
              } catch (e) {
                try {
                  const srvStr = execSync("ls /etc/init.d/ -m 2> /dev/null")
                    .toString()
                    .split("\n")
                    .join("");
                  srvString = "";
                      ...
                      curr_processes.forEach((element) => {
                        ...
                        if (resultProcess.pid) {
                          ...
                          for (let i in result) {
                            for (let j in result[i].pids) {
                              if (
                                parseInt(result[i].pids[j]) ===
                                parseInt(resultProcess.pid)
                              ) {
                                listPos = i;
                              }
                            }
              ...
              if (!error) {
                ...
                serviceSections.forEach((element) => {
                  if (element.trim() !== "") {
                    ...
                    let srvName = util
                      .getValue(lines, "Name", ":", true)
                      .toLowerCase();
                    let srvCaption = util
                      .getValue(lines, "Caption", ":", true)
                      .toLowerCase();
                    let started = util.getValue(lines, "Started", ":", true);
                    let startMode = util.getValue(
                      lines,
                      "StartMode",
                      ":",
                      true
                    ...
                    if (
                      srvString === "*" ||
                      srvs.indexOf(srvName) >= 0 ||
                      srvs.indexOf(srvCaption) >= 0
                    ) {
                      result.push({
                        name: srvName,
...
function processLoad(proc, callback) {
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      processesString = processesString
        .trim()
        .toLowerCase()
        .replace(/, /g, "|")
        .replace(/,+/g, "|");
      ...
      const procSanitized = util.isPrototypePolluted()
        ? ""
                ...
                if (!error) {
                  ...
                  processSections.forEach((element) => {
                    if (element.trim() !== "") {
                      ...
                      processes.forEach(function (proc) {
                        if (
                          name.toLowerCase().indexOf(proc.toLowerCase()) >= 0 &&
                          !inList
                        ) {
                          inList = true;
                          pname = proc;
                  ...
                  procStats.forEach((element) => {
                    ...
                    for (let j = 0; j < result.length; j++) {
                      if (
                        result[j].pid === resultProcess.pid ||
                        result[j].pids.indexOf(resultProcess.pid) >= 0
                      ) {
                        listPos = j;
                      }
                    }
              ...
              let lines = stdout
                ...
                .filter(function (line) {
                  ...
                  processes.forEach(function (item) {
                    found =
                      found ||
                      line.toLowerCase().indexOf(item.toLowerCase()) >= 0;
                  });
                  return found;
                });
              ...
              procStats.forEach(function (item) {
                ...
                for (let j = 0; j < result.length; j++) {
                  if (
                    item.name
                      .toLowerCase()
                      .indexOf(result[j].proc.toLowerCase()) >= 0
                  ) {
                    listPos = j;
                  }
                }
                processes.forEach(function (proc) {
                  if (
                    item.name.toLowerCase().indexOf(proc.toLowerCase()) >= 0 &&
                    !inList
                  ) {
                    inList = true;
                    name = proc;
